...
export function useContacts(activeTab: ContactStatus, searchQuery: string, favoritesOnly: boolean = false, interest: string = 'ALL', locationFilter: string = 'ALL', unattendedOnly: boolean = false) {
    ...
    const addInteraction = async (id: string, type: string, content: string, directedToId?: string | null, imageUrl?: string | null) => {
        try {
            if (type === 'STORE_VISIT') {
            ...
            const res = await fetch(`/api/contacts/${id}/interactions`, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ type, content, directedToId: directedToId || null, imageUrl: imageUrl || null })
            });
            if (res.ok) {
                await fetchContacts();
            } else {

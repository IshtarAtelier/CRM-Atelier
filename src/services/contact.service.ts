    ...
    async addPayment(orderId: string, amount: number, method: string, notes?: string, receiptUrl?: string, date?: Date | string) {
        return await prisma.$transaction(async (tx) => {
            ...
            if (receiptUrl) {
                ...
                // Aviso al admin con la imagen del comprobante (ventas web y de local)
                import('@/lib/receipt-notify').then(({ notifyReceiptUploaded }) =>
                    notifyReceiptUploaded({
                        ...
                        reference: notes,
                        receiptUrl,
                        context: 'Nuevo pago registrado'
                    })
                ).catch(err => console.error('[ReceiptNotify Background Error]', err));
            }

...
import {
    ...
    TrendingUp, Banknote, CreditCard, ArrowRightLeft
} from 'lucide-react';
...
export default function CotizadorPage() {
    ...
    const filtered = useMemo(() => {
        ...
        return products.filter(p => {
            ...
            if (activeType && activeType !== 'NONE' && (p.type || 'Otros') !== activeType) return false;
    ...
    const buildQuoteText = () => {
        const lines = quoteItems.map(
            item => `• ${item.product.brand || ''} ${item.product.model || item.product.name || ''} x${item.quantity} — $${(item.customPrice * item.quantity).toLocaleString()}`
        );
                ...
                <div className="flex-1 flex items-center gap-1.5 overflow-x-auto" style={{ scrollbarWidth: 'none' }}>
                    ...
                    {availableTypes.map(type => {
                        ...
                        return (
                            <button
                                key={type}
                                onClick={() => setActiveType(activeType === type ? 'NONE' : type)}
                                        ...
                                        {filtered.map((product, idx) => {
                                            ...
                                            return (
                                                ...
                                                >
                                                    ...
                                                    <td className="px-3 py-2 border-r border-stone-100 dark:border-stone-800">
                                                        <p className="text-[9px] font-bold text-stone-700 dark:text-stone-200 leading-tight max-w-[320px] truncate" title={product.name || ''}>
                                    ...
                                    {hasCrystals && (
                                        <div className="mb-3 p-3 bg-amber-50 dark:bg-amber-900/10 rounded-xl border-2 border-amber-200 dark:border-amber-800 flex items-start gap-4">
                                            ...
                                            <div className="flex-1 min-w-0">
                                                            ...
                                                            <div className="flex gap-2 overflow-x-auto" style={{ scrollbarWidth: 'thin' }}>
                                                                {frameResults.map(fr => (
                                                                    <button
                                                                        ...
                                                                        className="flex-shrink-0 flex items-center gap-2 p-2 bg-white dark:bg-stone-800 rounded-lg border border-stone-100 dark:border-stone-700 hover:border-amber-300 transition-all"
                                                                    >
                                                ...
                                                {frameSource === 'OPTICA' && hasFrameFromOptica && (
                                                    <p className="text-[10px] font-bold text-amber-700 dark:text-amber-400 flex items-center gap-1.5 py-1">

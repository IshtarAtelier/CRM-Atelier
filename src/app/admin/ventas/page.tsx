...
import { webPaymentGatewayLabel } from '@/lib/checkout/payment-gateway';
...
export default function VentasPage() {
    ...
    const autoSubmitSmartLab = async (order: any, pairNum: number = 1) => {
        ...
        try {
            ...
            const nearPdOd = order.labNearPdOd;
            const nearPdOi = order.labNearPdOi;
    ...
    const advanceStatus = async (orderId: string, currentStatus: string) => {
        ...
        try {
            ...
            alert(`⚠️ Error de red al avanzar el estado: ${error.message || error}`);
        }
        setUpdatingId(null);
    };
                                                    ...
                                                    try {
                                                        const res = await fetch('/api/whatsapp/send', {
                                                            ...
                                                            body: JSON.stringify({
                                                                ...
                                                                // Las _v3 llevan el horario real (9 a 20, sábados 9 a 17),
                                                                // corregido el 1/9/26 — las _v2 decían "8 a 20" (aprobadas
                                                                // el mismo día, quedaron viejas horas después).
                                                                template: financials.hasBalance
                                                                    ? { name: 'pedido_listo_saldo_v3', bodyParams: [(order.client?.name || 'cliente').split(' ')[0], `#${order.id.slice(-4).toUpperCase()}`, `$ ${financials.remainingCard.toLocaleString('es-AR')}`, `$ ${financials.remainingTransfer.toLocaleString('es-AR')}`, `$ ${financials.remainingCash.toLocaleString('es-AR')}`] }
                                                                    : { name: 'pedido_listo_v3', bodyParams: [(order.client?.name || 'cliente').split(' ')[0], `#${order.id.slice(-4).toUpperCase()}`] },
                                                            })
                                                        });
                                                        if (res.ok) {

...
export async function GET() {
  ...
  for (const c of contacts) {
    ...
    if (c.phone) orConditions.push({ phone: { contains: c.phone.slice(-8) } });
    if (c.dni) orConditions.push({ dni: c.dni });

    const found = orConditions.length > 0
      ? await prisma.client.findFirst({ where: { OR: orConditions } })
      : null;

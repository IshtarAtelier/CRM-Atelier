...
export async function POST(req: Request) {
  ...
  try {
    ...
    const paywayRequest = {
      ...
      currency: "ARS",
      // Débito: siempre 1 pago. Crédito: solo planes ofrecidos por el comercio,
      // 1/3/6 cuotas (los demás valores caen a 1 pago).
      installments: isDebit ? 1 : ([1, 3, 6].includes(parseInt(customer.installments || "1", 10)) ? parseInt(customer.installments || "1", 10) : 1),

...
/**
 * Resuelve el código corto del link (`?r=`) al id real del lead, para que el
 * tracking siga registrando quién abrió el catálogo. Si el código no matchea
 * —o matchea más de uno— devolvemos null: la página se muestra igual, solo
 * queda sin atribuir. Nunca hacemos fallar la vista por el tracking.
 */
async function resolveLeadByCode(code: string): Promise<string | null> {
  const clean = code.replace(/[^a-z0-9]/gi, '');
  if (clean.length < 6) return null;
  try {
    const matches = await prisma.opticaLead.findMany({
      where: { id: { endsWith: clean } },
      select: { id: true },
      take: 2,
    });
...
export default async function CatalogoMayoristaPage({
  ...
  // `?r=` es el código corto que arma el panel (últimos 8 caracteres del id del
  // lead) para que el link quede prolijo. `?lead=` con el id completo se sigue

...
export function OrderDetailPanel({ 
    ...
    const handleImportPrescription = (rxId: string, pair: 1 | 2) => {
        ...
        const imported = {
            ...
            prismOD: found.prismOD != null ? String(found.prismOD) : (pair === 1 ? rx1.prismOD : rx2.prismOD),
            prismOI: found.prismOI != null ? String(found.prismOI) : (pair === 1 ? rx1.prismOI : rx2.prismOI),
        ...
        return (
            <div className="space-y-4 bg-stone-50 dark:bg-stone-900/35 border border-stone-200/50 dark:border-stone-800 p-4 rounded-2xl">
                ...
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-2.5 pt-2 border-t border-stone-200/50 dark:border-stone-800">
                    ...
                    <div>
                        <label className="text-[8px] font-black text-stone-400 dark:text-stone-500 uppercase tracking-widest block mb-0.5">Prisma OD</label>
                        <input
                            type="text"
                            value={pairRx.prismOD || ''}
                            onChange={(e) => updateField('prismOD', e.target.value)}
                            placeholder="ej. 2 BN"
                            className="w-full text-[10px] p-2 border border-stone-200 dark:border-stone-700 bg-white dark:bg-stone-800 rounded-xl focus:outline-none focus:ring-1 focus:ring-amber-500"
                        />
                    </div>
                    <div>
                        <label className="text-[8px] font-black text-stone-400 dark:text-stone-500 uppercase tracking-widest block mb-0.5">Prisma OI</label>
                        <input
                            type="text"
                            value={pairRx.prismOI || ''}
                            onChange={(e) => updateField('prismOI', e.target.value)}
                            placeholder="ej. 2 BN"
                            className="w-full text-[10px] p-2 border border-stone-200 dark:border-stone-700 bg-white dark:bg-stone-800 rounded-xl focus:outline-none focus:ring-1 focus:ring-amber-500"
                        />
                    </div>

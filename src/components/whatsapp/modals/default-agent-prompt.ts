/**
 * Reglas base del asistente cuando la base todavía no tiene un prompt guardado.
 *
 * OJO (trampa conocida de CLAUDE.md): lo que el bot responde EN PRODUCCIÓN sale
 * de `SystemSetting.bot_prompt`, no de acá. Este texto es solo el punto de
 * partida que se muestra en el editor la primera vez; tocarlo y deployar NO
 * cambia lo que contesta un bot que ya tiene su prompt guardado.
 */

export const PROMPT_BASE_POR_DEFECTO = `Eres Ishtar, Óptico Contactólogo de Atelier Óptica. Atiendes a prospectos nuevos.

...
export function CopilotChat({ userName = 'Usuario', userRole = 'STAFF' }: CopilotChatProps) {
  ...
  // El disparador vive en la barra de Accesos (ver FloatingDock).
  useAbrirPanelRemoto('copilot', useCallback(() => setIsOpen(true), []));
  ...
  return (
    <>
      {/* El disparador vive en la barra de Accesos (FloatingDock), junto al
          de WhatsApp: todo junto en una píldora en vez de botones sueltos. */}

      {/* Chat Panel */}
      <div className={`fixed ${isWhatsApp ? 'top-[100px] right-6' : 'bottom-6 right-6'} z-[95] w-[420px] max-w-[calc(100vw-2rem)] transition-all duration-300 ease-out ${isOpen ? 'translate-y-0 opacity-100 scale-100' : 'translate-y-8 opacity-0 scale-95 pointer-events-none'}`}>

import { prisma } from '@/lib/db';

export type AuditAction = 'CREATE' | 'UPDATE' | 'DELETE' | 'STATUS_CHANGE' | 'LOGIN' | 'EXPORT' | 'NOTIFY' | 'PRICE_OVERRIDE' | 'ATTEMPT_FAILED' | 'GOOGLE_CONVERSION' | 'OTHER';
export type AuditEntityType =
    | 'ORDER' | 'CONTACT' | 'PAYMENT' | 'USER' | 'PRODUCT'
    | 'TASK' | 'PRESCRIPTION' | 'INVOICE' | 'EXPENSE' | 'DOCTOR_PAYMENT'
    | 'COUPON' | 'SETTING' | 'VENDOR_CASH' | 'POST_SALE_CASE'
    | 'CASH_HANDOVER' | 'CASH_COUNT' | 'CASH_MOVEMENT' | 'OPTICA_LEAD' | 'LAB_COST_ENTRY' | 'WEB_LENS_OPTION' | 'OTHER';

export async function logAudit(params: {
  userId?: string | null;
  userName?: string | null;
  action: AuditAction;
  entityType: AuditEntityType;
  entityId: string;
  details?: any;
}) {
  try {
    await prisma.auditLog.create({
      data: {
        userId: params.userId || null,
        userName: params.userName || 'Sistema',
        action: params.action,
        entityType: params.entityType,
        entityId: params.entityId,
        details: params.details || null,
      },
    });
  } catch (error) {
    console.error('Error al guardar audit log:', error);
    // No lanzamos el error para no romper el flujo principal si falla el log
  }
}

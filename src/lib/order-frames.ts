...
export function contarParesDeCristales(items: FrameItemLike[] | null | undefined): number {
    ...
    if (cristales.length === 0) return 0;
    ...
    if (od > 0 || oi > 0) {
        const sinOjo = cristales
            .filter(it => !it.eye)
            .reduce((n, it) => n + (it.quantity || 1), 0);
        return Math.max(od, oi) + Math.ceil(sinOjo / 2);
    }
...
export function cantidadDeArmazones(order: {
    ...
    const porCristales = contarParesDeCristales(order?.items);
    ...
    return Math.max(1, porCristales, esPromo2x1 ? 2 : 0);
}

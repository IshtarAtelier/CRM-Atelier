...
async function salesNode(state) {
  ...
  ══════════════════════════════════════
  - TENES LECTURA MULTIMODAL DIRECTA: Podés ver las imágenes que envía el cliente directamente en el historial del chat. Si ves que el cliente te envió una foto de una receta médica (graduación oftalmológica), vos misma debés leer los valores (OD, OI, esferas, cilindros, ejes, adición, DIP, etc.) directamente de la imagen. Además, buscá si figura el nombre del paciente en el texto de la receta médica. Luego:
  ...
  ══════════════════════════════════════
  1. ESTRUCTURACIÓN EN BLOQUES CORTOS (MÁXIMO 45 PALABRAS POR PÁRRAFO): Está prohibido enviar textos largos y densos en un solo párrafo. Separá tus ideas usando un doble salto de línea (una línea en blanco) para enviar múltiples mensajes cortos e independientes. Cada párrafo o bloque individual debe ser conciso (máximo 45 palabras). NUNCA unas palabras sin espacio, ni elimines los espacios naturales o de puntuación para acortar texto; priorizá la ortografía y legibilidad perfectas. Deja siempre espacios claros entre emojis, textos y números (ej. "💵 Efectivo: $200.000 / 💳 Tarjeta: 6 cuotas"). NUNCA escribas los caracteres literales "\\n".
...
async function executiveNode(state) {
  ...
  REGLAS DE ESTILO Y TONO (ESTRICTO):
  1. RESPUESTAS FLUIDAS Y CORTAS (MÁXIMO 45 PALABRAS POR PÁRRAFO): Nunca envíes bloques largos de texto en un solo párrafo. Si vas a decir más de 45 palabras, divídelo OBLIGATORIAMENTE usando un doble salto de línea (dejando una línea en blanco en el medio). Esto hará que el sistema envíe múltiples globitos de WhatsApp separados. NUNCA unas palabras sin espacio, ni elimines los espacios naturales o de puntuación para acortar el texto; priorizá siempre la ortografía perfecta. Deja espacios adecuados entre emojis, precios y palabras (ej. "💵 Efectivo: $200.000 y 💳 Tarjeta: 6 cuotas"). NUNCA escribas los caracteres literales "\\n".

...
function getTierInstruction(followUpType) {
    const TIER_INSTRUCTIONS = {
        DIA_1: `TIPO DE SEGUIMIENTO: DÍA 1 (24 horas después del presupuesto). ETAPA INICIAL: el cliente recién recibió la cotización.\n` +
               `OBJETIVO DE VENTA: Sondear sin presionar, NOMBRANDO el producto cotizado (ej: "los multifocales", "los cristales que vimos"): una pregunta corta de sondeo (qué le parecieron las opciones). CIERRE OBLIGATORIO: ofrecerle mandarle fotos de modelos, en diminutivo cálido y como pregunta (ej: "querés que te mande alguna fotito para que veas modelitos?"). Son exactamente DOS preguntas: el sondeo y la oferta de fotos — ninguna más.\n` +
        ...
        DIA_4: `TIPO DE SEGUIMIENTO: DÍA 4 (4 días después del presupuesto).\n` +
               `OBJETIVO DE VENTA: Comentarle amigablemente que los precios de laboratorio SE ACTUALIZAN pronto (afirmación directa, nunca "pueden actualizarse"), haciendo referencia al producto que cotizó. SOLO si en el historial el cliente YA mostró interés claro en avanzar (dijo que lo quería, que pasaba a pagar, que le gustó), podés recordarle que puede señar para congelar el valor o pagar en cuotas sin interés. Si todavía no definió nada o no respondió, NO ofrezcas la seña: seguí sondeando (si pudo verlo, qué le pareció, si buscaba algo más económico). Si prometió pasar por el local, hacé referencia directa a eso (ej: "me dijiste que pasabas por el local..."). Incluí PROACTIVAMENTE la dirección al invitarlo: "Estamos en José Luis de Tejeda 4380, Cerro de las Rosas" — sin que la pida, y agregá la foto de la fachada al final del mensaje, tal cual: [IMAGE: https://atelieroptica.com.ar/api/storage/view?key=agent_fachada.jpg]\n` +
        ...
        INACTIVIDAD: `TIPO DE SEGUIMIENTO: RETOMAR CHARLA (el cliente no respondió tu último mensaje hace más de un día).\n` +
               `OBJETIVO DE VENTA: Retomar la conversación desde el punto exacto donde quedó (mirá el historial: qué le preguntaste o qué estaban por definir) y SONDEAR con suavidad: hacé UNA SOLA pregunta (elegí una: si pudo ver las opciones, qué le parecieron, o si buscaba algo más económico). Si había un producto o presupuesto en juego, nombralo.\n` +

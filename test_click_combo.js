...
async function main() {
  ...
  const data = await page.evaluate(async () => {
    ...
    async function tryOpen(lblTxt) {
      ...
      if(!box) return "No box found";
      
      // Try mousedown instead of click
      ...
      if(!lb) {
        // Try clicking parent
        box.parentElement.dispatchEvent(new MouseEvent('mousedown', {bubbles:true, cancelable:true, view:window}));
        await new Promise(r => setTimeout(r, 1000));
    ...
    results.aro = await tryOpen('Tipo de aro');
    results.armazon = await tryOpen('Tipo armazón');
    return results;
  });

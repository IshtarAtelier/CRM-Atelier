...
| ~~B13~~ | ~~301 de `promo.atelieroptica.com.ar` al dominio principal~~ · **RESUELTO el 25/9/2026** en Cloudflare (registro `promo` en *Proxied* + Redirect Rule 301 a `https://atelieroptica.com.ar/`, conserva gclid/utm) | Antes: HTTP 200, sin noindex, promos no verificables (2x1, 15% OFF) en la SERP de marca | — | — |
| B14 | Desindexar CRM railway.app de la SERP de marca | ya tiene noindex+canonical; desde el 25/9/2026 además redirige siempre por Host al dominio real (`next.config.ts`, `dab79414`). Queda opcional el removal en Search Console para acelerar | S | Higiene de marca |

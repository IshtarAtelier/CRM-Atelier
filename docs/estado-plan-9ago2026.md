...
| **B14** desindexar CRM railway | `X-Robots-Tag: noindex` por host ya existía. `src/middleware.ts:21-26,207-209` | **Actualizado 25/9/2026:** el redirect por host a `atelieroptica.com.ar` quedó **siempre prendido** en `next.config.ts` (commit `dab79414`; la variable `DOMAIN_CUTOVER` ya no existe — nunca pudo funcionar porque el Dockerfile no la pasaba al build). Solo falta, si se quiere acelerar, el removal en Search Console | S |
...
| ~~**301 de `promo.atelieroptica.com.ar`**~~ — **✅ RESUELTO el 25/9/2026** en Cloudflare, sin Wave: registro A `promo` en *Proxied* + Redirect Rule "promo viejo -> sitio real (301)" → `https://atelieroptica.com.ar/` conservando la query (gclid/utm) | B13 |

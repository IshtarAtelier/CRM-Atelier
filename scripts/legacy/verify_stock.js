...
const orderItems = [
  ...
  { model: "GS7015S", color: "C3", qty: 5 },
  { model: "GS7015S", color: "C1", qty: 5 },
...
async function main() {
  ...
  for (const item of orderItems) {
    ...
    if (matches.length === 0) {
      ...
      // Check if any match has correct stock
      const hasCorrectStock = matches.some(p => p.stock === item.qty || p.stock >= item.qty);
      if (!hasCorrectStock) {
        wrongStock.push({ item, found: matches.map(m => ({ name: m.name, model: m.model, stock: m.stock })) });
        allCorrect = false;
      }
    }
  }

  console.log("Missing Products in DB:", missing.length);
  if (missing.length > 0) console.log(missing.slice(0, 5));
  
  console.log("\nProducts with wrong stock:", wrongStock.length);
  if (wrongStock.length > 0) console.log(JSON.stringify(wrongStock.slice(0, 5), null, 2));
  
  if (allCorrect) {
    console.log("All stock is verified and correct!");
  }
}

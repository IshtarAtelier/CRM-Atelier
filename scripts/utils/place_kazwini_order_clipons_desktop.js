...
async function main() {
  const folderPath = '/Users/ishtarpissano/Desktop/Pedido Clipon';
  const files = fs.readdirSync(folderPath).filter(f => f.endsWith('.avif'));
  
  const groups = {};
  files.forEach(f => {
    // Parse " 7036 - c2.avif", "8125S c4.avif", "R12221 .avif", etc.
    let name = f.replace('.avif', '').trim();
    ...
    groups[base].push({ color, fullModel: name });
  });

  const baseCodes = Object.keys(groups);
  console.log(`Parsed ${files.length} images into ${baseCodes.length} base models.`);
  baseCodes.forEach(b => console.log(`- ${b}: [${groups[b].map(v => v.color).join(', ')}]`));
    ...
    try {
      ...
      const matchedCardIndex = await page.evaluate((targetBaseCode) => {
        ...
        return cards.findIndex(card => {
          ...
          return codeText.toLowerCase().includes(targetBaseCode.toLowerCase());
        });
      }, baseCode);
  ...
  console.log("\nAll clipons processed. Navigating to the shopping cart page...");

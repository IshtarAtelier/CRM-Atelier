...
async function main() {
    ...
    console.log("Actualizando VARILUX COMFORT MAX...");
    const comfortMax = await prod.product.findMany({
        where: { name: { contains: 'VARILUX COMFORT MAX' } }
    });
    
    for (const p of comfortMax) {
    ...
    console.log("\nActualizando VARILUX COMFORT...");
    const comfort = await prod.product.findMany({
        where: { name: { contains: 'VARILUX COMFORT -' } }
    });
    
    for (const p of comfort) {
        let sMax = 0, sMin = -10.0, cMax = 6.0, cMin = -6.0;
